export const detailedAssessments: Record<string, any> = {
  'Breast cancer': {
    ...
    evaluate: (answers: Record<string, boolean>) => {
      if (answers.history || answers.brca || answers.radiation) return { status: 'high-risk', message: 'HIGH RISK IDENTIFIED: Standard average-risk guidelines do not apply. Patient meets criteria for specialized screening pathways (e.g., Breast MRI) and requires immediate specialist consultation.' };
